...
from bridge_config import bridge_service_settings, bridge_settings
...
def query_response_from_params(params: dict[str, list[str]]) -> tuple[int, dict | list[dict]]:
    ...
    try:
        limit = max(1, int(limit_raw or SERVICE_DEFAULT_LIMIT))
    except ValueError:
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if parsed.path == "/query":
            try:
                ...
                self._write_json(HTTPStatus.INTERNAL_SERVER_ERROR, {"error": "query failed"})
                return
...
def main() -> None:
    ...
    logger.info("Bridge query service listening on http://%s:%d", service.host, service.port)
    try:
