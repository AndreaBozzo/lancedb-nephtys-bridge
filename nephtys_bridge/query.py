...
from .db import open_table_if_exists
from .utils import extract_inline_field
...
def _filter_results(
    ...
    allowed_sources = {
        source.strip() for source in source_filters or [] if source.strip()
    }
    allowed_event_types = {
        event_type.strip()
        for event_type in event_type_filters or []
        if event_type.strip()
    }
    allowed_symbols = {
        symbol.strip() for symbol in symbol_filters or [] if symbol.strip()
    }
    ...
    for row in raw_results:
        ...
        event_type = str(
            row.get("event_type") or extract_inline_field(text, "type") or ""
        )
...
def _serialize_result(row: dict) -> dict:
    ...
    return {
        ...
        "datetime": datetime.fromtimestamp(timestamp_ms / 1000).strftime(
            "%Y-%m-%d %H:%M:%S"
        ),
...
def print_results(
    results: list[dict], query_text: str, content_only: bool, json_output: bool
) -> None:
    if json_output:
...
def cli_main() -> None:
    parser = argparse.ArgumentParser(
        description="Query semantic search results from LanceDB"
    )
    ...
    print_results(
        results, args.query, content_only=not args.all_namespaces, json_output=args.json
    )
