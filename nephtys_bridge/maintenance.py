...
from .config import bridge_maintenance_settings, bridge_settings
...
def run_maintenance(
    ...
    return {
        "deleted_runs": prune_e2e_runs(
            e2e_root, keep_runs=keep_runs, max_age_days=max_age_days
        ),
...
def cli_main() -> None:
    parser = argparse.ArgumentParser(
        description="Apply retention and compaction policies"
    )
    ...
    parser.add_argument(
        "--cleanup-older-than-days", type=float, default=RETENTION_VERSION_AGE_DAYS
    )
