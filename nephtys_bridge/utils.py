...
def extract_text_content(
    source_id: str, event_type: str, item: dict[str, Any]
) -> str | None:
    ...
    volume = (
        _clean_str(item.get("volume"))
        or _clean_str(item.get("qty"))
        or _clean_str(item.get("q"))
    )
...
def upgrade_legacy_row(row: dict[str, Any]) -> dict[str, Any] | None:
    ...
    source_id = (
        _clean_str(row.get("source_id"))
        or extract_inline_field(text, "source")
        or "unknown"
    )
    event_type = (
        _clean_str(row.get("event_type"))
        or extract_inline_field(text, "type")
        or "event"
    )
    symbol = (
        _clean_str(row.get("symbol")) or extract_inline_field(text, "symbol") or None
    )
